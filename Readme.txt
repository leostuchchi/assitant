Проект astra является частью проекта personal_assistant состоящего из проектов (с отдельными репозиториями):
...
astra: 
    ...
    добавить  !!!данные только для работы моделей !!! пользователю не выводятся !!! в удобном для модели виде (например [0, 1]):
    1. добавить magic_profile: 
    создание и запись в базу данных натального и психоматричного профиля пользователя на основе расчетов: 
    добавляемые расчеты проекта:
...
ИНТЕГРИРОВАННАЯ СТРУКТУРА ДЛЯ МОДЕЛИ
{
  "psychological_blueprint": {
    ...
    2. добавить расчеты и запись в бд для проекта assistant: расчитывает оптимальные по энергетике 3 активности из 6 + финансы, 
    3. проверить точность производимых расчетов, оптимизировать без добавления шума
    4. проверить оптимальность потоков вычислений
    5. создать новый sql для бд
    6. проверить оптимальность изменений и интеграции в существующий проект
    
 СПИСОК МОДУЛЕЙ ДЛЯ СОЗДАНИЯ

    backend/magic_profile.py - расчет психологических профилей

    backend/activity_optimizer.py - расчет оптимальных активностей

    backend/model_data_formatter.py - преобразование данных для моделей

    backend/calculation_validator.py - проверка точности расчетов

    tests/calculation_accuracy_test.py - тесты точности

📋 СПИСОК МОДУЛЕЙ ДЛЯ ИЗМЕНЕНИЯ

    backend/assistant.py - добавление новых методов

    backend/database.py - новые модели SQLAlchemy

    backend/prediction_services.py - интеграция с magic profile

    bot/handlers.py - новые команды для профилей

    backend/user_services.py - расширение пользовательских данных

    docker-compose.yml - обновление при необходимости
    
    
testing: 
    имеет точку входа для тестирования, 
    психологическое тестирование с расчетом результатов, 
    запись в бд.
    
семь сервисов shared ollama: 
     physical, 
     spiritual, 
     learning, 
     psychological, 
...
assistant: 
    имеет точку входа, 
    при отсутствии расчетов перенаправление в другие проекты, 
    имеет доступ ко всем бд, 
    расчитывает оптимальные по энергетике 3 активности из 6 + финансы, 
    на основании трех оптимальных активностей делает запросы к sharel ollama.

qest: имеет точку входа, опросник потребностей и желаний пользователя.



структура проекта astra(существующая, для переделки):
